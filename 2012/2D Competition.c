...
/*
 ...
 Last Modified: 2012-09-30 23:31:28.0
 */
...
#define lim 0.25f
#define printDebug 0
#define sixtyDeg 1.0471f
#define stillDis 0.05f
#define stillVel 0.01f
	vec zeroVec;
	bool red;
...
#define stateVel(state) (state + 3)
	...
	void shoot(vec myPos, vec targetPos, cmd shootcmd) {
		vec att;
		attToTarget(myPos, targetPos, att);
		float angle = angBetweenVecs(stateAtt(myState), att);
#if printDebug
		DEBUG(("Attitude Targeting Info\n"));
		printVec("\tTarget Pos (m)", targetPos);
		printVec("\tTarget Att (unitvec)", att);
		DEBUG(("\tAngle (rad)\t%.3f\n", angle));
#endif
    ...
    //END::PAGE::ctrl
    //BEGIN::PAGE::game
	void smartGo (float x, float yvel) {
		float v = x - statePos(myState)[0];
		if (v > 0.015f) v = 0.015f;
		if (v < -0.015f) v = -0.015f;
		vec desiredVel = {v, yvel, floatZero};
		api.setVelocityTarget(desiredVel);
	}
	...
	bool hitTarget(ZRState satState, vec targetPos) {
		return (mathVecMagnitude(stateVel(satState), 3) < stillVel && disBetweenPts(statePos(satState), targetPos) < stillDis);
	}
